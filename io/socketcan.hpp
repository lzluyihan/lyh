...
public:
  SocketCAN(const std::string &interface,
      ...
      : interface_(interface), socket_fd_(-1), epoll_fd_(-1),
        rx_handler_(rx_handler), quit_(false), ok_(false) {
    try_open();
...
private:
  ...
  void read() {
    ...
    if (num_events == -1)
      throw std::runtime_error("Error wating for events!");

    for (int i = 0; i < num_events; i++) {
      ssize_t num_bytes =
          recv(socket_fd_, &frame_, sizeof(can_frame), MSG_DONTWAIT);
      if (num_bytes == -1)
