  ...
  std::vector<cv::Mat> & tvecs)
{
  ...
  for (int i = 1; true; i++) {
    ...
    // 修复1：resize传参错误导致的崩溃（显式cv::Size()，兼容所有OpenCV版本）
...
int main(int argc, char * argv[])
{
  ...
  // 修复2：仅将相机旋转向量转为3x3旋转矩阵（calibrateHandEye硬性要求：旋转输入必须是矩阵）
  std::vector<cv::Mat> R_cam2world;
  for (const auto& rvec : rvecs) {
      cv::Mat R;
      cv::Rodrigues(rvec, R); // 旋转向量→3x3旋转矩阵，仅此一处格式转换
      R_cam2world.push_back(R);
  }

  // 手眼标定：100%回归你的原始调用，无任何修改
  cv::Mat R_camera2gimbal, t_camera2gimbal;
  cv::calibrateHandEye(
    R_gimbal2world_list, t_gimbal2world_list, R_cam2world, tvecs,
    R_camera2gimbal, t_camera2gimbal);
  t_camera2gimbal /= 1e3;  // mm to m

  // 计算相机同理想情况的偏角：100%回归你的原始代码，矩阵/欧拉角顺序全不变
